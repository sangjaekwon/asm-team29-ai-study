...
- substitutions와 additional_ingredients는 Agent4가 입력으로 준 값만 그대로 사용하세요. 새 대체 재료나 추가 재료를 판단해서 만들지 마세요.
...
def _fallback_recommendation_reasons(
    ...
    return [f"{sentence}." for sentence in sentences] if len(sentences) > 1 else [selected_reason]
...
def _coerce_recommendation_reasons(value: Any, fallback: list[str]) -> list[str]:
    ...
    return _unique_items(reasons)[:5] or fallback
...
def _build_fallback_recipe(
    ...
    return GeneratedRecipe(
        ...
        additional_ingredients=[
            item for item in state.get("additional_ingredients", []) if str(item or "").strip()
        ],
...
def _call_solar_recipe_generator(
    ...
    parsed["substitutions"] = _coerce_substitutions(state.get("substitutions", []))
    parsed["additional_ingredients"] = [
        item for item in state.get("additional_ingredients", []) if str(item or "").strip()
    ]
